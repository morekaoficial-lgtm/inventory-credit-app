...
app.post('/api/sync/update-offices', async (_req, res) => {
    try {
        // Get all unique SKUs from receptions
        const skusResult = await database_1.pool.query(`
      SELECT DISTINCT sku FROM receptions WHERE sku IS NOT NULL AND sku != ''
    `);
        const skus = skusResult.rows.map((r) => r.sku);
        let updated = 0;
        let errors = 0;
        let notFound = 0;
        for (const sku of skus) {
            try {
                // Find variant in Bsale
                const variant = await bsale.getVariantBySku(sku);
                if (!variant) {
                    notFound++;
                    continue; // Skip SKUs not found in Bsale
                }
                ...
                // Update all receptions for this SKU (and set variant_id too)
                const result = await database_1.pool.query(`
          UPDATE receptions 
          SET office_id = $1, office_name = $2, variant_id = $3
          WHERE sku = $4
        `, [officeId, officeName, variant.id, sku]);
...
app.get('/api/report/:sku/excel', async (req, res) => {
    try {
        ...
        const ultimasRcText = result.ultimasRcPrecioViejo.map(r => `${r.documento || 'S/INV'} | ${r.fecha || 'N/A'} | ${r.sucursal || 'N/A'}`).join('\n');
        const wsData = [
...
async function buildCreditNotesExcel(status, folio) {
    const conditions = [];
    const params = [];
    if (status) {
        params.push(status);
        conditions.push(`cn.status = $${params.length}`);
    }
    if (folio) {
        params.push(folio);
        conditions.push(`cn.folio = $${params.length}`);
    }
  ...
  `, params);
    ...
    `, [sku]);
        ...
        const ultimasRcText = lastThreeOld.map((r) => `${r.document_number || 'S/INV'} | ${r.admission_date ? new Date(r.admission_date * 1000).toISOString().split('T')[0] : 'N/A'} | ${r.office_name || 'N/A'}`).join('\n');
        ...
        try {
            ...
            if (variant) {
                ...
                stockPorSucursalText = stocks.map((s) => `${s.officeName}: ${s.quantityAvailable}`).join('\n');
