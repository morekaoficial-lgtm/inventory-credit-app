...
export async function saveCreditNotes(notes: CreditNoteInput[], pdfId?: number): Promise<void> {
...
export async function markCreditNotePaid(id: number): Promise<void> {
  await pool.query(`UPDATE credit_notes SET status = 'paid' WHERE id = $1`, [id]);
}

export async function getCreditNoteSummary() {
  const pending = await pool.query(`SELECT COALESCE(SUM(amount), 0) as total FROM credit_notes WHERE status = 'pending'`);
